...
public static class FriendsSeeder
{
    public static async Task SeedAsync(AppDbContext context)
    {
        ...
        // Create some user follows (for users who follow but aren't friends)
        for (int i = 0; i < Math.Min(users.Count, 8); i++)
        {
            for (int j = i + 1; j < Math.Min(users.Count, 8); j++)
            {
                // Only create follows if they're not already friends
                ...
                if (!existingFriendship && Random.Shared.Next(0, 3) == 0) // 33% chance
                {
