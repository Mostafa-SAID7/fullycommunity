...
public class TwoFactorService : ITwoFactorService
{
    ...
    public TwoFactorService(AppDbContext context, INotificationService notificationService)
    {
        _context = context;
    ...
    public async Task<bool> ValidateEmailCodeAsync(Guid userId, string code)
        => await ValidateOtpAsync(userId, code, "Email");

    public async Task<bool> ValidateSmsCodeAsync(Guid userId, string code)
        => await ValidateOtpAsync(userId, code, "SMS");
    ...
    public async Task SendEmailOtpAsync(Guid userId)
    {
        ...
        var otp = GenerateOtp();
        await SaveOtpAsync(userId, otp, "Email");

        await _notificationService.SendTemplatedEmailAsync("otp-verification", user.Email!, 
    ...
    public async Task SendSmsOtpAsync(Guid userId)
    {
        ...
        var otp = GenerateOtp();
        await SaveOtpAsync(userId, otp, "SMS");
    ...
    public async Task<IEnumerable<string>> GenerateRecoveryCodesAsync(Guid userId, int count = 10)
    {
        var existingCodes = await _context.TwoFactorBackupCodes
            .Where(c => c.UserId == userId && !c.IsUsed).ToListAsync();
        _context.TwoFactorBackupCodes.RemoveRange(existingCodes);
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            codes.Add(code);
            await _context.TwoFactorBackupCodes.AddAsync(new TwoFactorBackupCode
            {
                Id = Guid.NewGuid(), UserId = userId, Code = HashCode(code), CreatedAt = DateTime.UtcNow
            });
        }
        await _context.SaveChangesAsync();
        return codes;
    ...
    public async Task<int> GetRemainingRecoveryCodesCountAsync(Guid userId)
        => await _context.TwoFactorBackupCodes.CountAsync(c => c.UserId == userId && !c.IsUsed);
    ...
    public async Task<TwoFactorStatus> GetTwoFactorStatusAsync(Guid userId)
    {
        ...
        return new TwoFactorStatus(
            user.TwoFactorEnabled, user.TwoFactorType, !string.IsNullOrEmpty(user.TwoFactorSecret),
            user.EmailConfirmed, user.PhoneNumberConfirmed, await GetRemainingRecoveryCodesCountAsync(userId));
    }
    ...
    private async Task<bool> ValidateOtpAsync(Guid userId, string code, string type)
    {
        var otp = await _context.OtpCodes.FirstOrDefaultAsync(o => 
            o.UserId == userId && o.Code == code && o.Type == type && !o.IsUsed && o.ExpiresAt > DateTime.UtcNow);
    ...
    private async Task SaveOtpAsync(Guid userId, string code, string type)
    {
        var existingOtps = await _context.OtpCodes.Where(o => o.UserId == userId && o.Type == type && !o.IsUsed).ToListAsync();
        _context.OtpCodes.RemoveRange(existingOtps);

        await _context.OtpCodes.AddAsync(new OtpCode
        {
            Id = Guid.NewGuid(), UserId = userId, Code = code, Type = type,
            ExpiresAt = DateTime.UtcNow.AddMinutes(OtpExpirationMinutes), CreatedAt = DateTime.UtcNow
        });
        await _context.SaveChangesAsync();
    }

    private static string GenerateOtp() => RandomNumberGenerator.GetInt32(100000, 999999).ToString();
    private static string GenerateRecoveryCode() => Convert.ToHexString(RandomNumberGenerator.GetBytes(5)).ToLower();
    private static string HashCode(string code) => Convert.ToBase64String(SHA256.HashData(Encoding.UTF8.GetBytes(code)));
    private static string FormatManualEntryKey(string key) => string.Join(" ", Enumerable.Range(0, key.Length / 4).Select(i => key.Substring(i * 4, 4)));
}
